...
def process_queue_element(
    orchestrator_connection: OrchestratorConnection, queue_element: QueueElement
):
    ...
    if agreement is None:
        ...
        orchestrator_connection.log_error(
            f"{system_name = } with {service_name = } and {current_status = } not found in agreements for {info_dict['institutionsnr']}"
        )
        raise BusinessError()
    ...
    orchestrator_connection.log_trace(
        f"Handling agreement: {system_name = } with {service_name = } and {current_status = } for {info_dict['institutionsnr']}"
    )

    if wanted_status in ["GODKENDT", "VENTER"]:
        status_resp = change_status(
            orchestrator_connection, reference, agreement, session
        )

        info_dict["status respons"] = status_resp.text

        orchestrator_connection.log_trace(
            f"Status successfully changed. Info: {info_dict}"
        )
        ...
        orchestrator_connection.log_trace(
            f"Agreement successfully deleted. Info: {info_dict}"
        )
